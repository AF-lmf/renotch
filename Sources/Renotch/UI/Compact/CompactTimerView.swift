...
struct CompactTimerView: View {
    ...
    var body: some View {
        ...
        .accessibilityLabel(timer.isActive ? (timer.isPaused ? "\(timer.currentMode.title)计时已暂停" : "\(timer.currentMode.title)计时进行中") : "番茄钟已就绪")
    ...
    private var subtitleText: String {
        if timer.isActive {
            if timer.isPaused { return "点按以管理" }
            if timer.currentMode == .focus && (timer.storedTimer?.isAutoAdvance ?? timer.autoAdvance) {
                ...
                return "随后休息 \(breakM) 分钟"
            }
            return "进行中"
        }
        return "就绪 · \(timer.focusMinutes) 分钟 + \(timer.breakMinutes) 分钟"
    }
