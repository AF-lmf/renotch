...
struct TimerView: View {
    ...
    private var timerPicker: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack(spacing: 8) {
                // Focus Card
                durationCard(
                    title: PomodoroMode.focus.title,
                    icon: "timer",
                ...
                // Break Card
                durationCard(
                    title: PomodoroMode.breakTime.title,
    ...
    private func durationCard(
        ...
        VStack(alignment: .leading, spacing: 6) {
            ...
            HStack(spacing: 4) {
                ...
                HStack(spacing: 2) {
                    Button {
                        let delta = title == PomodoroMode.focus.title ? (minutes > 5 ? -5 : -1) : -1
                        onAdjust(delta)
                    } label: {
                    ...
                    Button {
                        let delta = title == PomodoroMode.focus.title ? 5 : 1
                        onAdjust(delta)
                    } label: {
        ...
        VStack(spacing: 8) {
            ...
            HStack(spacing: 16) {
                ...
                VStack(spacing: 5) {
                    ...
                    HStack(spacing: 4) {
                        SmallActionButton(
                            title: timer.currentMode == .focus ? PomodoroMode.breakTime.title : PomodoroMode.focus.title,
                            icon: "forward.fill"
                        ) {
                            timer.skip()
                        }
    ...
    private var activeSubtitle: String {
        ...
        case .focus:
            ...
            return "时间到后将自动开始 \(bMin) 分钟的休息。"
        case .breakTime:
            return "放松一下，休息结束时会通知你。"
        }
    }
}
