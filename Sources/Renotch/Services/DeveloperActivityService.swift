    ...
    nonisolated private static func collectSnapshot() async -> ActivityCollectionSnapshot {
        ...
        if let git {
            activities.append(DeveloperActivity(
                ...
                subtitle: git.changedFiles == 0 ? "\(git.branch) · 无更改" : "\(git.changedFiles) 个更改 · \(git.branch)",
    ...
    static func scan(processes: [RunningProcess]) async -> [DeveloperActivity] {
        ...
        for (pid, port) in candidates {
            ...
            results.append(DeveloperActivity(
                ...
                detail: directory?.path ?? "通过本地监听端口检测到"
            ))
        }
    ...
    static func scan(processes: [RunningProcess]) -> [DeveloperActivity] {
        processes.compactMap { process in
            ...
            return DeveloperActivity(
                ...
                workingDirectory: directory,
                detail: directory?.path ?? "通过正在运行的本地进程检测到"
            )
        }
    }
    ...
    static func scan(candidateDirectories: [URL]) -> GitActivitySnapshot? {
        ...
        for directory in candidates {
            ...
            return GitActivitySnapshot(
                ...
                branch: branch.isEmpty ? "分离头指针" : branch,
